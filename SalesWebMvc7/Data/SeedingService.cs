...
{
    public class SeedingService     // serviço de popular a base de dados
    {
        private SalesWebMvc7Context _context;

        // registro de uma dependencia desse serviço com o dbcontext

        public SeedingService(SalesWebMvc7Context context)
        {
            _context = context;
            // injeçao de dependencia
            // o _context da classe recebe o context do argumento
        }

        public void Seed()
        {
            var teste = _context.Sellers.Select(c => c.Name).ToList();

            if (_context.Department.Any() ||
            _context.Sellers.Any() ||         // mudei sellers
            ...
            Sellers s1 = new(1, "Bob Brown", "bob@gmail.com", new DateTime(1998, 4, 21), 1000.0, d1);
            ...
            _context.Department.AddRange(d1, d2, d3, d4);          // AddRange() permite adicionar varios objetos de uma vez só

            _context.Sellers.AddRange(s1, s2, s3, s4, s5, s6);   // mudei sellers
            ...
            //Todas as mudanças feitas no contexto não serão persistidas no banco de dados até que seja invocado o método SaveChanges() do mesmo.

            // pronto. feita a inclusão dos objetos no banco de dados - serviço de popular a base de dados
